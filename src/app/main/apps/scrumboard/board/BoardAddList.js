...
function BoardAddList(props) {
  ...
  function onSubmit(data) {
    dispatch(newList({ boardId: board._id, listTitle: data.title }));
    handleCloseForm();
  }

  return (
    <div>
      {user_connected.role === "admin" ? (
        <Card
          className={clsx(
            classes.card,
            "w-320 mx-8 sm:mx-12 rounded-20 shadow"
          )}
          square
        >
          {formOpen ? (
            ...
            >
              ...
              <span className="mx-8">Ajouter une List</span>
            </Button>
          )}
        </Card>
      ) : null}
