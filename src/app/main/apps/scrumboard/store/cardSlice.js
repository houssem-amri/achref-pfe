...
export const updateCard = createAsyncThunk(
  ...
  async ({ boardId, card }, { dispatch }) => {
    ...
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/card/update",
      {
        boardId,
        card,
      }
...
export const removeCard = createAsyncThunk(
  ...
  async ({ boardId, cardId }, { dispatch }) => {
    console.log("cardIdcardIdcardIdcardId", { boardId, cardId });

    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/card/remove",
      {
        boardId,
        cardId,
      }
