  ...
  async (params, { dispatch }) => {
    try {
      const response = await axios.get(
        "http://localhost:3200/api/scrumboard-app/board",
        { params }
      );
      const data = await response.data.board;
      console.log("here etape 3", data);
      return data;
    } catch (error) {
      dispatch(
        showMessage({
...
export const reorderList = createAsyncThunk(
  ...
  async (result, { dispatch, getState }) => {
    ...
    const response = axios.post(
      "http://localhost:3200/api/scrumboard-app/list/order",
      {
        boardId: board._id,
        lists: ordered,
      }
...
export const reorderCard = createAsyncThunk(
  ...
  async ({ source, destination }, { dispatch, getState }) => {
    ...
    console.log("hellllo", ordered);

    let response = {};
    response = axios.post(
      "http://localhost:3200/api/scrumboard-app/card/order",
      {
        boardId: board._id,
        lists: ordered,
      }
...
export const newCard = createAsyncThunk(
  ...
  async ({ boardId, listId, cardTitle }, { dispatch, getState }) => {
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/card/new",
      {
        boardId,
        listId,
...
export const newList = createAsyncThunk(
  ...
  async ({ boardId, listTitle }, { dispatch, getState }) => {
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/list/new",
      {
        boardId,
    ...
    const data = await response.data.boards;
    console.log("here newList 3 data", data);
...
export const renameList = createAsyncThunk(
  ...
  async ({ boardId, listId, listTitle }, { dispatch, getState }) => {
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/list/rename",
      {
        boardId,
        listId,
        listTitle,
...
export const removeList = createAsyncThunk(
  ...
  async ({ boardId, listId }, { dispatch, getState }) => {
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/list/remove",
      {
        boardId,
        listId,
      }
...
export const changeBoardSettings = createAsyncThunk(
  ...
  async (newSettings, { dispatch, getState }) => {
    ...
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/board/settings/update",
      {
        boardId: board._id,
        settings,
      }
...
export const deleteBoard = createAsyncThunk(
  ...
  async (boardId, { dispatch, getState }) => {
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/board/delete",
      {
        boardId,
      }
    );
...
export const copyBoard = createAsyncThunk(
  ...
  async (board, { dispatch, getState }) => {
    console.log("board copieee", board);
    const newBoardData = _.merge({}, board, {
      ...
      super_user_id: board.super_user_id,
    });
...
export const renameBoard = createAsyncThunk(
  ...
  async ({ boardId, boardTitle }, { dispatch, getState }) => {
    const response = await axios.post(
      "http://localhost:3200/api/scrumboard-app/board/rename",
      {
        boardId,
        boardTitle,
      }
...
const boardsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [removeList.fulfilled]: (state, action) => {
      state.lists = _.reject(state.lists, { _id: action.payload });
    },
    ...
    [updateCard.fulfilled.type]: (state, action) => {
      state.cards = state.cards.map((_card) => {
        if (_card._id === action.payload._id) {
