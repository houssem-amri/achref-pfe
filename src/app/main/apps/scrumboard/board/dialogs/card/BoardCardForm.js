...
function BoardCardForm(props) {
  ...
  const list = card
    ? _.find(board.lists, (_list) => _list.idCards.includes(card._id))
    : null;
  ...
  useEffect(() => {
    ...
    if (!_.isEqual(card, cardForm)) {
      updateCardData(board._id, cardForm);
    }
  }, [board._id, card, cardForm, updateCardData]);
          ...
          <Toolbar className="flex w-full overflow-x-auto px-8 sm:px-16">
            <div className="flex flex-1">
              ...
              <OptionsMenu
                onRemoveCard={() =>
                  dispatch(
                    removeCard({ boardId: board._id, cardId: cardForm._id })
                  )
                }
              />
            </div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                ...
                fullWidth
                required
                disabled={user_connected.role === "admin" ? false : true} 
                InputProps={{
                  endAdornment: (
          ...
          {cardForm.idMembers && cardForm.idMembers.length > 0 && (
            <div className="flex-1 mb-24 mx-8">
              ...
              <Autocomplete
                ...
                value={cardForm.idMembers.map((_id) =>
                  _.find(board.members, { _id })
                )}
                onChange={(event, newValue) => {
                  setValue(
                    "idMembers",
          ...
          cardForm.checklists.map((checklist, index) => (
            <CardChecklist
              ...
              onRemoveCheckList={() => {
                setValue(
                  "checklists",
                  _.reject(cardForm.checklists, { _id: checklist._id })
                );
              }}
            />
          ))}
